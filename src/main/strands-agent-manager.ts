...
- ですます調は使わず、である調で記述してください
- できるだけ Owner (主にリードしている SA や関係者のエイリアス) の情報を入れてください
- Next step に関しては、Owner & Timeline を必ず入れてください
- Ask については、「できることはやっている、でもまだ足りないのでサポートをしてほしい」といった内容を書いてください
- STARフレームワーク（Situation, Task, Action, Result）を意識して記述してください
- Situation は定量的な情報を含めるようにしてください
- ビジネスインパクトを明確かつ定量的に記載してください
- お客様の個人名よりは職務を記載してください

サンプル:
[Game8 (Gunosy) - Bedrockを用いた掲示板監視機能が複数掲示板で利用開始, ARR $12k] 日本最大級のゲーム総合情報Webサービスを運営するGame8では、ゲームタイトル毎に掲示板を運営している。この掲示板は監視ができておらず、不適切な投稿がされた場合は主にエンドユーザからの通報にて削除の対応を行っている（規模感として約1,000件/日の削除対応を行っている）。この対応を自動化すべくBedrockで不適切な投稿を検出できないかという相談をCTOより頂いた。[Action Taken] 投稿に対してタグ付けするサンプルのプロンプトを作成し提供を行った。また、掲示板の責任者、エンジニア、CTOを集めて掲示板の監視に関するプロンプトチューニングを実施した。このディスカッションで掲示板ごとに検知すべき単語 / しない単語が分かれているなどゲームのドメイン知識が多分に求められる領域であることがディスカッションの中で浮き彫りになり、ゲームタイトルごとにプロンプトを作るというアプローチとなった。まずは掲示板のうち特に荒れていた"鳴潮" 1タイトルのみでテスト利用を行い、評価後に本番利用という流れとしていたが、別タイトルのディレクターからも利用したいとの声が上がり、７タイトルでの本番導入が始まった。[Next Action] 11月中旬まで効果測定を行うこととしており、その結果を共有していただけることとなっている。また、事例化の合意も取れており、効果測定後に事例化を進める予定としている。Owner: kntnag@

上記のサンプルをよく参考にして、以下の情報をもとに週次報告書を作成してください:
- 顧客名: ${customerName}
- 案件名: ${opportunityName || '不明'}
- 案件規模: ${opportunitySize || '不明'}
